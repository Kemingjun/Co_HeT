...
def get_options(args=None):
    ...
    parser.add_argument('--use_moe', action='store_true', default=True,
                        help='Use sparse MoE layers in the AM encoder and decoder projections')
    parser.add_argument('--no_moe', action='store_false', dest='use_moe',
                        help='Disable sparse MoE layers and recover the original AM architecture')
    parser.add_argument('--moe_num_experts', type=int, default=8, help='Number of experts in each MoE layer')
    parser.add_argument('--moe_top_k', type=int, default=2, help='Number of selected experts per token')
    parser.add_argument('--moe_aux_loss_weight', type=float, default=0.01,
                        help='Weight of the MoE load-balancing auxiliary loss')
    parser.add_argument('--moe_position', default='encoder_decoder', choices=['encoder', 'decoder', 'encoder_decoder'],
                        help='Where to insert MoE layers')

    # Training
    parser.add_argument('--lr_model', type=float, default=1e-4, help="Set the learning rate for the actor network")
    parser.add_argument('--lr_critic', type=float, default=1e-4, help="Set the learning rate for the critic network")
    parser.add_argument('--lr_decay', type=float, default=1.0, help='Learning rate decay per epoch')
    parser.add_argument('--train_precision', default='32', choices=['32', 'bf16', 'fp16'],
                        help='Training precision: 32, bf16 or fp16. Default keeps the original FP32 path.')
    ...
    assert opts.grad_accumulation_steps >= 1, "grad_accumulation_steps must be >= 1"
    return opts
