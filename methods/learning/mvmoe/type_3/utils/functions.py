...
def load_model(path, epoch=None):
    ...
    model = model_class(
        ...
        moe_num_experts=args.get('moe_num_experts', 4),
        moe_top_k=args.get('moe_top_k', 2),
        moe_position=args.get('moe_position', 'encoder_decoder')
    )
...
def sample_many(inner_func, input, batch_rep=1, iter_rep=1):
    """
    ...
    for i in range(iter_rep):
        _log_p, pi, cost, distance, tardiness = inner_func(input)
