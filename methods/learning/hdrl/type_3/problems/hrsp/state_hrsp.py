...
class StateHRSP(NamedTuple):
    ...
    def initialize(input, visited_dtype=torch.uint8):
        ...
        route = torch.full(
            ...
            -1, dtype=torch.long, device=coords.device
        )

        route_len = torch.zeros(batch_size, paramet_hrsp.ROBOT_NUM, dtype=torch.long, device=coords.device)
    ...
    def update(self, selected_task, selected_robot_one_hot):
        ...
        robot_op_time = op_time_for_selected_task.gather(1, paramet_hrsp.robot_type_indices.to(updated_time.device).expand(updated_time.size(0), -1))
        ...
        b_idx, r_idx = selected_robot_one_hot.nonzero(as_tuple=True)  # [K], [K]
        pos = route_len[b_idx, r_idx]  # [K]
        t_idx = selected_task[b_idx]  # [K]

        route[b_idx, r_idx, pos] = t_idx
        route_len[b_idx, r_idx] = route_len[b_idx, r_idx] + 1
    ...
    def get_routes(self):
        ...
        Returns:
            routes: list[list[list[int]]]
                routes[b][r]  batch brobot r Python 
        """
        routes = []
        for b in range(self.route.size(0)):
            rb = []
            for r in range(self.route.size(1)):
                L = int(self.route_len[b, r].item())
                rb.append(self.route[b, r, :L].tolist())
            routes.append(rb)
        return routes
