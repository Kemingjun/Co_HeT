...
class StateHRSP(NamedTuple):
    ...
    def update(self, selected_task, selected_robot_one_hot):
        ...
        robot_op_time = op_time_for_selected_task.gather(1, paramet_hrsp.robot_type_indices.to(updated_time.device).expand(updated_time.size(0), -1).to(cur_coord.device))
