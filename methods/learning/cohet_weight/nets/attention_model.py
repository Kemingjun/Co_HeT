...
class AttentionModel(nn.Module):
    ...
    def _inner(self, input, node_embeddings):
        ...
        travel_time = sum_length.unsqueeze(-1) / paramet_hrsp.ROBOT_VELOCITY
        cost = travel_time * paramet_hrsp.WEIGHT + state.tardiness * (1 - paramet_hrsp.WEIGHT)
    ...
    def robot_decision(self, robot_embeddings, selected_task_node_emb):
        ...
        for robot_type_index in range(paramet_hrsp.ROBOT_TYPE_NUM):
            robot_type_log_p, robot_type_embeddings = self._get_log_p_robot(robot_embeddings, selected_task_node_emb, robot_type_index, selected_robots_embedding_list)
