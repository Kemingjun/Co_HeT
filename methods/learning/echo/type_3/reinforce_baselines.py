...
class RolloutBaseline(Baseline):
    ...
    def _update_model(self, model, epoch, dataset=None):
        ...
        if dataset is not None:
            if len(dataset) != self.opts.val_size:
                ...
                dataset = None
            elif self._get_instance_size(dataset[0]) != self.opts.graph_size:
    ...
    @staticmethod
    def _get_instance_size(instance):
        if isinstance(instance, dict):
            for key in ('loc', 'source', 'supply_norm'):
                if key in instance:
                    return instance[key].size(0)
            raise KeyError("Cannot infer graph size from dataset instance")
