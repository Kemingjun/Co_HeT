...
class StateHRSP(NamedTuple):
    # Fixed input
    coords: torch.Tensor            # 所有任务的坐标
    deadline: torch.Tensor          # 所有任务的截止时间
    operation_time: torch.Tensor    # 所有任务的执行时间
    ...
    # prev_task: torch.Tensor   # 车的前一个任务    [512, VEHICLE_NUM]
    cur_time: torch.Tensor    # 车当前的时间      [512, VEHICLE_NUM]
    cur_coord: torch.Tensor   # 车当坐标          [512, VEHICLE_NUM, 2]
    visited_: torch.Tensor    # 全局信息
    length: torch.Tensor      # 车的行驶距离      [512, VEHICLE_NUM]
    tardiness: torch.Tensor   # 各个车的迟到时间  [512, VEHICLE_NUM]
    ...
    def initialize(input, visited_dtype=torch.uint8):
        coords = input['source']  # 此处没有包含depot

        batch_size, n_loc, _ = coords.size()


        # prev_task = torch.zeros(batch_size, ROBOT_NUM, dtype=torch.long, device=coords.device)  # 前一个任务其实不重要了 TODO 可以删掉
        ...
        return StateHRSP(
            ...
            visited_=(  # Visited as mask is easier to understand, as long more memory efficient
                ...
                torch.zeros(
                    batch_size, 1, n_loc,  # 注：这里不包括depot
    ...
    def update(self, selected_task, selected_robot_one_hot):
        ...
        Args:
            ...
            selected_robot_one_hot: [batch_size, robot_num]  true 选中   false 未选中
        ...
        # 计算每个 batch 中的最大值（注意 keepdim=True 保留维度）
        max_time_per_batch, _ = masked_time.max(dim=1, keepdim=True)
        # 将最大值广播到 [B, R]，仅赋值给被选中的 robot
        ...
        robot_type_indices = paramet_hrsp.robot_type_indices.to(op_time_for_selected_task.device)
