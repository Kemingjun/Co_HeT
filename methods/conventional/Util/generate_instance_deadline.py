...
def generate_instance(size, ROBOT_TYPE_NUM, base=0.5):
    instance = []
    ddl_base = [i * base + base for i in range(size)]
    noise = [random.uniform(-0.2, 0.2) for _ in range(size)]
    ddl_list = [b + n for b, n in zip(ddl_base, noise)]
    random.shuffle(ddl_list)

    points = generate_points(size, dist_type="uniform")
...
if __name__ == "__main__":
    size = 20
    ROBOT_TYPE_NUM = 2
    for base in [0.3, 0.4, 0.6, 0.7]:
        for i in range(1, 21):
            file_name = f"N{size}_K{ROBOT_TYPE_NUM}_M12_{base}_deadline/N{size}_K{ROBOT_TYPE_NUM}_M12_{str(base)[-1]}_deadline_I{i}"
            instance = generate_instance(size, ROBOT_TYPE_NUM, base=base)
