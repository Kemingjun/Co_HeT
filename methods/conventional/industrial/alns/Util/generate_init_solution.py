...
def generate_solution_random(instance):
    ...
    for task in range(1, task_num + 1):
        for robot_type in range(1, Config.ROBOT_TYPE_NUM + 1):
            if required_robot(instance[task - 1])[robot_type - 1] == 1:
...
def generate_solution_greedy(instance):
    ...
    task_time_list = [[task, deadline(instance[task - 1])] for task in task_list]
    ...
    for task_time in task_time_list:
        ...
        required_robot_list = required_robot(instance[destroyed_task - 1])
        search_insertions(
            instance=instance,
            required_robot=required_robot_list,
...
def generate_solution_greedy_random_shuffle(instance):
    ...
    for task in task_list:
        ...
        required_robot_list = required_robot(instance[task - 1])
        search_insertions(
            instance=instance,
            required_robot=required_robot_list,
            current_robot_type=1,
...
def generate_solution_nearest(instance, factor=0.9):
    ...
    while len(task_list) != 0:
        ...
        task_cost_list = []
        for task in task_list:
            task_deadline = deadline(instance[task - 1])
            task_position_x, task_position_y = handover_pos(instance[task - 1])
            cost = 0
        ...
        for robot in robot_list:
            ...
            robot_position_x = robot_state_map[robot][1][0]
            robot_position_y = robot_state_map[robot][1][1]
            selected_handover = handover_pos(instance[selected_task - 1])
            distance = math.fabs(robot_position_x - selected_handover[0]) + math.fabs(robot_position_y - selected_handover[1])
            arrival_time = robot_time + distance / Config.CARRIER_VELOCITY
        ...
        for type, robot in enumerate(robot_list):
            ...
            robot_state_map[robot][0] = start_time
            robot_state_map[robot][1] = handover_pos(instance[selected_task - 1])
