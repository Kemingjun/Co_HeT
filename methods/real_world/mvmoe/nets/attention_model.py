...
from nets.moe import TopKMoELinear
...
class AttentionModel(nn.Module):
    ...
    def __init__(self,
        ...
        self.graph_embedder = GraphAttentionEncoder(
            ...
            use_moe=use_moe and moe_position in ('encoder', 'encoder_decoder'),
        ...
        if use_moe and moe_position in ('decoder', 'encoder_decoder'):
            self.project_out = TopKMoELinear(embedding_dim, embedding_dim, moe_num_experts, moe_top_k, bias=False)
            self.project_out_robot = TopKMoELinear(embedding_dim, embedding_dim, moe_num_experts, moe_top_k, bias=False)
        else:
    ...
    def forward(self, input, return_pi=False):
        ...
        self.moe_aux_loss = self._collect_moe_aux_loss(node_embeddings)
    ...
    def _collect_moe_aux_loss(self, reference_tensor):
        aux_losses = []
        for module in (self.graph_embedder, self.project_out, self.project_out_robot):
            if hasattr(module, 'moe_aux_loss'):
                aux_losses.append(module.moe_aux_loss.to(reference_tensor.device))
        return sum(aux_losses) / len(aux_losses) if aux_losses else reference_tensor.new_tensor(0.0)
