...
from nets.GRU import EnvGRUUpdater, VehicleGRUUpdater
...
class AttentionModel(nn.Module):
    ...
    def __init__(self,
        ...
        self.envGRUUpdater = EnvGRUUpdater(embedding_dim, hidden_dim)
        self.vehicleGRUUpdater = VehicleGRUUpdater(paramet_hrsp.ROBOT_NUM, embedding_dim, hidden_dim)
        ...
        node_dim = 7  # supply xy, handover xy, delivery xy, normalized deadline
        ...
        self.context_update_linear = nn.Linear(paramet_hrsp.ROBOT_NUM * 3, embedding_dim, bias=False)
        # self.robot_update_linear = nn.Linear(robot_dim - paramet_hrsp.ROBOT_TYPE_NUM, embedding_dim, bias=False)
        ...
        """
        璁や负涓ゅrobot_embedder鐨勫姛鑳戒笉鍚岋紝涓婇潰鐨勬槸涓轰簡閫夋嫨浠诲姟浣滀负query鐨勪竴閮ㄥ垎锛岃€屼笅闈㈢殑鏄负浜嗛€夋嫨杞﹁締
        """
        self.robot_type_embedder = RobotTypePromptEncoder(
            n_heads=n_heads,
            embed_dim=embedding_dim,
            n_layers=1,
            normalization='layer'
        ...
        self.env_context_init = nn.Linear(2 * embedding_dim, embedding_dim, bias=False)
    ...
    def forward(self, input, return_pi=False):
        ...
        _log_p, pi, cost, distance, tardiness = self._inner(input, node_embeddings, graph_embedding)
    ...
    def robot_update_embed(self, state):
        ...
        B, m, f = robot_init_info.size()  # 1024, 12, 3
        C_en_t_info = robot_init_info.reshape(B, m * f)
        return C_en_t_info
    ...
    def _inner(self, input, node_embeddings, graph_embedding):
        ...
        env_context_info = torch.cat([graph_embedding, robot_embd_sum], dim=-1)

        env_context = self.env_context_init(env_context_info)
        ...
        while not state.all_finished():
            ...
            task_log_p, mask = self._get_log_p_task(node_embeddings, graph_embedding, selected_robots_embedding_mean, state)
            ...
            # robot_embeddings, robot_embd_sum = self.robot_embedder(self.robot_init_embed(state))
            # GRU鏇存柊
            C_en_t_info = self.robot_update_embed(state)
            C_en_t = self.context_update_linear(C_en_t_info)
            # robot_states_mean = robot_states.mean(dim=1)
            # new_context = self.context_update_linear(robot_states_mean)

            env_context = self.envGRUUpdater(C_en_t, env_context)
            robot_embeddings = self.vehicleGRUUpdater(robot_embeddings, C_en_t_info)
    ...
    def sample_many(self, _input, batch_rep=1, iter_rep=1):
        ...
        return sample_many(
            ...
            (_input, graph_embeddings[0], graph_embeddings[1]),  # Pack input with embeddings (additional input)
            batch_rep, iter_rep
        )
    ...
    def _get_log_p_robot(self, robot_embeddings, env_context, robot_type_index, selected_robots_embedding_list, normalize=True):
        ...
        query = self.norm_query_robot(query + env_context.unsqueeze(1))
