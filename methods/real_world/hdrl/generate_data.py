...
def generate_cvrptw_data(dataset_size, vrp_size):
    ...
    depot_coords = np.random.uniform(size=(dataset_size, 2))  # depot: (B, 2)
    node_coords = np.random.uniform(size=(dataset_size, vrp_size, 2))  # nodes: (B, N, 2)
    demand = np.random.randint(1, 10, size=(dataset_size, vrp_size))  # demand: (B, N)
    capacity = np.full(dataset_size, CAPACITIES[vrp_size])  # capacity: (B,)

    # --- compute deadline ---
    # 1. broadcast depot to shape (B, N, 2)
    depot_expanded = depot_coords[:, None, :]

    # 2. compute Euclidean distance from depot to each node
    distances = np.linalg.norm(node_coords - depot_expanded, axis=2)  # shape: (B, N)

    margin = np.random.uniform(0.4, 0.8, size=(dataset_size, vrp_size))

    # 4. deadline = distance + margin
    deadline = distances + margin  # shape: (B, N)

    # return full tuple
    return list(zip(
        depot_coords.tolist(),  # depot
        node_coords.tolist(),  # node locs
        demand.tolist(),  # demand
        capacity.tolist(),  # capacity
        deadline.tolist()  # NEW: deadline
    ))
