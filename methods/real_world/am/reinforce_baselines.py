...
def get_baseline_dataset_graph_size(dataset):
    sample = dataset[0]
    if torch.is_tensor(sample):
        return sample.size(0)
    for key in ("loc", "source", "supply_raw", "supply_norm", "handover_raw", "delivery_raw"):
        if key in sample:
            return sample[key].size(0)
    raise KeyError(
        "Cannot infer graph size from baseline dataset sample. "
    ...
    def _update_model(self, model, epoch, dataset=None):
        ...
        if dataset is not None:
            if len(dataset) != self.opts.val_size:
                ...
                dataset = None
            elif get_baseline_dataset_graph_size(dataset) != self.opts.graph_size:
