...
class AttentionModel(nn.Module):
    ...
    def task_edge_coords(self, input):
        return (input['supply_norm'] + input['handover_norm'] + input['delivery_norm']) / 3.0

    def _encode_tasks(self, input):
        graph_input = self.graph_init_embed(input)
        if self.checkpoint_encoder and self.training:
            if self.use_echo_encoder:
                return checkpoint(self.graph_embedder, graph_input, self.task_edge_coords(input), use_reentrant=False)
            return checkpoint(self.graph_embedder, graph_input, use_reentrant=False)
        if self.use_echo_encoder:
            return self.graph_embedder(graph_input, self.task_edge_coords(input))
    ...
    def _inner(self, input, node_embeddings):
        ...
        while not state.all_finished():
            ...
            mask = state.get_mask()
            node_embeddings_with_mask = self.project_node_mask(torch.cat((task_embeddings_for_step, mask.transpose(1, 2)), dim=-1))  # 鎷兼帴鍦ㄦ渶鍚庝竴缁?
